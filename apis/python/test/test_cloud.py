...
class CloudTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ivf_flat_random_sampling_index_uri = f"{test_path}/test_cloud_ivf_flat_random_sampling_index"
    ...
    def tearDownClass(cls):
        ...
        vs.Index.delete_index(uri=cls.small_flat_index_uri, config=tiledb.cloud.Config())
        vs.Index.delete_index(uri=cls.small_vamana_index_uri, config=tiledb.cloud.Config())
        vs.Index.delete_index(uri=cls.small_ivf_flat_index_uri, config=tiledb.cloud.Config())
    ...
    def run_small_cloud_test(self, index_uri, index_type):
        ...
        index = vs.FlatIndex(
            uri="tiledb://paris-morgan/s3://tiledb-paris/tiledb/groups/zzz_unittest_vector_search_veoFMYBrPi/test_cloud_small_flat_index",
            config=tiledb.cloud.Config().dict(),
        )

        queries = np.array([data[1]], dtype=np.float32)
        
        print('[test_cloud@run_small_cloud_test] vs.query() mode = None ----------------------------')
        ...
        print('[test_cloud@run_small_cloud_test] vs.query() mode = LOCAL ----------------------------')
    ...
    #     self.run_small_cloud_test(
    #         CloudTests.small_vamana_index_uri, "VAMANA"
    #     )
