...
def test_construct_IndexVamana():
    ...
    a = vspy.IndexVamana(feature_type="float32", id_type="int64", max_candidates=11)
    assert a.max_candidates() == 11

    a = vspy.IndexVamana(feature_type="float32", id_type="int64", max_connections=22)
    assert a.max_connections() == 22

    a = vspy.IndexVamana(
        feature_type="float32", id_type="int64", max_candidates=11, max_connections=22
    )
    assert a.max_candidates() == 11
    assert a.max_connections() == 22
...
def test_construct_IndexVamana_with_empty_vector(tmp_path):
    ...
    assert a.max_candidates() == max_candidates
    assert a.max_connections() == max_connections
