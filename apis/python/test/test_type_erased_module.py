...
def test_numpy_to_feature_vector_array_data_types():
    for dtype in [np.float32, np.int8, np.uint8, np.int32, np.uint32, np.int64, np.uint64]:
        for dtype_ids in [np.uint32, np.uint64]:
            ...
            assert np.array_equal(vectors, np.array(feature_vector_array)), f"Arrays were not equal for dtype: {dtype}, dtype_ids: {dtype_ids}"
