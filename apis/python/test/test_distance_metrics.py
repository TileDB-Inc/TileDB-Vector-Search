...
def test_ivf_flat_create_cosine_numpy(tmp_path):
    ...
    index_uri_sum_of_squares = os.path.join(tmp_path, "sift10k_flat_sum_of_squares")
    index_sum_of_squares = ingest(
        index_type="IVF_FLAT",
        index_uri=index_uri_sum_of_squares,
        input_vectors=input_vectors,
        distance_metric=vspy.DistanceMetric.SUM_OF_SQUARES,
    )

    distances_sum_of_squares, ids_sum_of_squares = index_sum_of_squares.query(query_vectors, k=k)

    # Verify that L2 results are different from cosine results
    assert not np.array_equal(
        ids, ids_sum_of_squares
    ), "Cosine and L2 queries returned the same indices"

    for i, query in enumerate(query_vectors):
        for j, idx in enumerate(ids_sum_of_squares[i]):
            manual_distance = sum_of_squares_distance(query, input_vectors[idx])
            np.testing.assert_allclose(
                manual_distance,
                distances_sum_of_squares[i][j],
                rtol=1e-5,
                atol=1e-8,
    ...
    Index.delete_index(uri=index_uri, config={})
    Index.delete_index(uri=index_uri_sum_of_squares, config={})
...
def test_flat_l2_distance(tmp_path):
    ...
    assert np.allclose(distances_l2_sklearn, distances, rtol=1e-5, atol=1e-8), "L2 distances do not match"
...
def test_ivf_flat_l2_distance(tmp_path):
    ...
    assert np.all(np.diff(distances, axis=1) >= 0), "Distances are not sorted in ascending order"
